...
export const DoctorDashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-8 font-sans">
      {/* Header bar */}
      <div className="bg-gradient-to-r from-indigo-900/30 to-indigo-950/15 border border-indigo-800/40 p-6 rounded-2xl flex justify-between items-center shadow-lg">
        <div>
          ...
          <h2 className="text-xl font-black text-white mt-3">EMR & Care Team Workspace</h2>
          <p className="text-slate-400 text-xs mt-1">Manage active caseloads and prescriptions</p>
        </div>
        <button
          onClick={logout}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-slate-900/50 border border-slate-800/80 p-6 rounded-2xl shadow-sm space-y-4">
          <h3 className="text-xs font-black text-indigo-400 uppercase tracking-wider">Active Caseload</h3>
          <div className="space-y-3">
            {activePatients.map((p: any) => (
              ...
              >
                <div className="flex justify-between items-start">
                  ...
                  <span className="text-[9px] bg-indigo-500/10 text-indigo-400 border border-indigo-500/25 px-1.5 py-0.5 rounded font-black uppercase">
            ...
            {activePatients.length === 0 && (
              <p className="text-xs text-slate-500 italic text-center py-6">No active patients assigned.</p>
            )}
          </div>
        </div>
          ...
          {selectedPatient ? (
            <div className="bg-slate-900/50 border border-slate-800/80 p-6 rounded-2xl shadow-sm space-y-6">
              ...
              <div className="border-b border-slate-800 pb-4">
                ...
                <p className="text-xs text-slate-400 mt-1 font-mono">{selectedPatient.upid || "No Universal Identity record linked."}</p>
              </div>
              ...
              {loadingHistory ? (
                ...
                <div className="grid grid-cols-2 gap-6 text-xs text-slate-300 bg-slate-950 p-4 rounded-xl border border-slate-800">
                  <div>
                    ...
                    {historyData.allergies.length > 0 ? (
                      <div className="flex flex-wrap gap-1">
                        {historyData.allergies.map((a: string, i: number) => (
                          <span key={i} className="bg-red-500/10 text-red-400 border border-red-500/20 px-2 py-0.5 rounded text-[9px] font-bold">{a}</span>
                        ))}
                      </div>
                  ...
                  <div>
                    ...
                    {historyData.chronic_conditions.length > 0 ? (
                      <p className="font-semibold">{historyData.chronic_conditions.join(", ")}</p>
                    ) : <p className="italic text-slate-500">No chronic conditions listed.</p>}
                  </div>
                </div>
              ) : null}
                ...
                <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                      ...
                      <strong className="text-sm font-black text-rose-500">{selectedPatient.vitals.hr} bpm</strong>
                    </div>
                    <div>
                      <span className="text-[9px] text-slate-500 font-bold uppercase block">BP</span>
                      <strong className="text-sm font-black text-indigo-400">{selectedPatient.vitals.bp} mmHg</strong>
                    </div>
                    <div>
                      <span className="text-[9px] text-slate-500 font-bold uppercase block">SpO2</span>
                      <strong className="text-sm font-black text-emerald-400">{selectedPatient.vitals.o2}</strong>
                    </div>
                  </div>
                </div>
              )}
              ...
              {/* Treatment editor */}
              <form onSubmit={handleUpdateEMR} className="space-y-4 pt-4 border-t border-slate-800">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-400 uppercase">Physician SOAP Clinical Notes</label>
                  <textarea
                    rows={4}
                    required
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-400 uppercase">Order Prescription / Medications</label>
                  <input
                    type="text"
                ...
                <button
                  type="submit"
                  className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded-xl text-xs transition duration-150"
                >
